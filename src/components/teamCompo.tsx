...
export default function TeamComponent({path, name, description, style, href}: teamCard) {
  return (
    <Link href={href} className="block h-full">
      <div className={`${style} flex flex-col items-center p-2 sm:p-3 lg:p-4 rounded-xl shadow-lg transition-all duration-150 hover:shadow-2xl hover:brightness-110 hover:scale-105 cursor-pointer h-full min-h-0 backdrop-blur-md bg-opacity-30 hover:bg-opacity-40`}>
        <Image src={path} alt={name} width={96} height={96} className="w-16 sm:w-20 md:w-24 h-16 sm:h-20 md:h-24 mb-1 sm:mb-2 lg:mb-2 transition-transform duration-150 hover:scale-110" />
